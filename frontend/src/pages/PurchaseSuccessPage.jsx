...
const PurchaseSuccessPage = () => {
  ...
  useEffect(() => {
    const handle = async (sessionId) => {
      try {
        // Use axios instance so auth refresh interceptors work if access token expired during checkout
        const r = await axios.post('/payments/checkout-success', { sessionId })
        if (!r?.status || r.status >= 300) throw new Error(r?.data?.message || r?.data?.error || 'Failed to confirm checkout')
  ...
  if (isProcessing) return <div className='h-screen flex items-center justify-center text-gray-300'>Processing…</div>
  if (error) return <div className='h-screen flex items-center justify-center text-red-400'>Error: {error}</div>
